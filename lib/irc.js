...
const DIRECT_IRC_HOST = 'irc.irc-hispano.org';
const DIRECT_IRC_PORT = 6697;
const KIWI_REGISTER_WATCHDOG_MS = parseInt(process.env.KIWI_REGISTER_WATCHDOG_MS || '6500', 10);
const KIWI_CONNECT_TIMEOUT_MS = parseInt(process.env.KIWI_CONNECT_TIMEOUT_MS || '14000', 10);
...
class IRCClient extends EventEmitter {
  ...
  async connect() {
    ...
    // Fallback de robustez: si Kiwi falla por filtros de IP/gateway, intentar IRC TLS directo.
    try {
      this.emit('status', 'Kiwi no disponible; probando IRC TLS directo...');
      await this._tryDirectTls();
      this._connecting = false;
      return;
    } catch (tlsErr) {
      this.emit('status', `Fallback TLS fallido: ${tlsErr.message}`);
    }
    ...
    if (!this._destroyed) {
      ...
      throw new Error(
        `No se pudo conectar por Kiwi ni por TLS directo (posible filtro de IP). Ultimo estado: ${this._transport || 'sin transporte'}`
      );
    }
  }
  ...
  async _tryDirectTls() {
    if (this._activeProxy) {
      throw new Error('TLS directo no soporta SOCKS en este modo');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        reject(new Error('timeout en IRC TLS directo'));
      }, 20000);
      ...
      this.emit('status', `Conectando a ${DIRECT_IRC_HOST}:${DIRECT_IRC_PORT} (TLS)...`);

      const sock = tls.connect({
        host: DIRECT_IRC_HOST,
        port: DIRECT_IRC_PORT,
      ...
      this.ws = null;
      this._transport = 'tls';
      this._rawSender = (payload) => sock.write(payload);

      sock.setEncoding('utf8');

      sock.once('secureConnect', () => {
        this.emit('status', 'TLS directo establecido, registrando en IRC...');
        this._register();
      });
      ...
      sock.on('close', () => {
        if (!settled) {
          ...
          clearTimeout(timeout);
          this.removeListener('connected', onRegistered);
          reject(new Error('socket TLS cerrada antes del registro IRC'));
          return;
        }
        if (runtimeReady && this.connected) {
  ...
  _handleRuntimeClose() {
    ...
    this._reconnectTimer = setTimeout(() => {
      if (this._destroyed || this._connecting) return;
      this.emit('status', 'Reconectando sesión IRC...');
  ...
  quit(msg = 'Bye') {
    ...
    setTimeout(() => {
      ...
      try { this.tls && this.tls.destroy(); } catch (_) {}
      this.ws = null;
      this.tls = null;
    }, 500);
  }
