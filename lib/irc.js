...
class IRCClient extends EventEmitter {
  ...
  _onSockJSFrame(frame, resolve, opened, onOpen) {
    if (frame === 'o') {
      // Frame de apertura SockJS
      // El proxy KiwiIRC requiere "CONTROL START" antes de los comandos IRC
      onOpen();
      // Enviar CONTROL START sin \r\n (protocolo interno KiwiIRC, no IRC)
      if (this.ws && this.ws.readyState === WebSocket.OPEN) {
        this.ws.send(JSON.stringify([':kiwi.chathispano.com CONTROL START']));
      }
      // Pequeña espera para que el proxy procese el CONTROL START
